...
--------------------------------------
function plan_class:add_mts_obj(mts, pos, replacements)
	local size = mts.size
	local size_x = size.x
	local size_y = size.y
	local size_z = size.z
...
--------------------------------------
function plan_class:add_we_obj(we, pos, replacements)
	if type(pos) ~= "table" then pos = {} end
	local ox = pos['x'] or 0
	local oy = pos['y'] or 0
	local oz = pos['z'] or 0
...
function plan_class:read_from_schem_file(filename, replacements)
	...
	if mts_format then
		...
		print("read_from_schem_file", filename, mts.node_names)
	...
	elseif string.find(filename, '.we',   -3) or string.find(filename, '.wem',  -4) then
		...
		print("read_from_schem_file", filename, we.data)
